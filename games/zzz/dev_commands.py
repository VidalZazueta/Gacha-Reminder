...
def register_zzz_dev_commands(client) -> None:
    ...
    async def test_zzz_parsing(interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            embed = discord.Embed(title=f"Parsing Test: {TEST_EVENT}", color=discord.Color.blue())
            ...
            if date_result:
                ...
                if found:
                    embed.add_field(name="Found Date Patterns", value=f"```{', '.join(found[:5])}```", inline=False)
    ...
    async def test_2025_event(interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            embed = discord.Embed(title=f"2025 Event Test: {TEST_EVENT}", color=discord.Color.green())
            ...
            embed.add_field(name="Content Preview", value=f"```{content[:300]}```", inline=False)
    ...
    async def search_recent_zzz(interaction: discord.Interaction) -> None:
        try:
            ...
            for year, label, emoji in (("2025", "2025 Events", "🎉"), ("2024", "2024 Events", "📅"), ("other", "Other Events", "❓")):
                ...
                embed.add_field(name=f"{emoji} {label} ({len(items)})", value=value, inline=False)
